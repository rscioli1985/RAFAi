...
4. **Render secrets templates (optional but recommended)**
   ```bash
   ./scripts/render-secrets.sh
   ```
   - Copies `infra/secrets/templates` into `$SECRETS_DIR` so backend+Airflow share Reddit/LLM credentials.

5. **(Optional) Launch Airflow orchestrator container**
   ```bash
...
6. **(Optional) Run the job reconciler worker**
   ```bash
...
7. **(Optional) Start Prometheus monitoring stack**
   ```bash
